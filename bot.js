...
bot.on("ready", () => {
	console.log("Aboose Caboose Ready!");
}

bot.on("message", (msg) => {
	if (!message.author.bot) {
		if (message.content.toLowerCase().indexOf('aboos') !== -1 || message.content.toLowerCase().indexOf('abuse') !== -1) {
			message.channel.sendMessage(message.author + ' INVOKED THE _**__ABOOS CABOOSE__**_\n' + photos[parseInt(Math.random() * photos.length)]);
		}
	}
});

bot.login('MjIwMTU4NjM4NzMwMzc5MjY1.CqcVrw.Fj2IMYmzt5CAzhotO-ZVBtU0DSc');
